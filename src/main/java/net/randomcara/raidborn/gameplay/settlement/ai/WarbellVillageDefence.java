...
class WarbellVillageDefence {
    ...
    private static boolean shouldDrop(Mob mob, BlockPos bellPos, int radius, LivingEntity target) {
        return !target.isAlive() || target.isRemoved() || SettlementBridge.isProtectedVillageTarget(mob, target) || WarbellVillageData.isPositionOutsideVillage(bellPos, radius, target.blockPosition(), WarbellVillageData.COMBAT_LEASH_BUFFER)
    ...
    private static LivingEntity findIntruder(Mob mob, BlockPos bellPos, int radius) {
        ...
        return mob.level().getEntitiesOfClass(LivingEntity.class, scanBox, entity -> entity != mob && entity.isAlive() && isIntruder(entity) && !WarbellVillageData.isPositionOutsideVillage(bellPos, radius, entity.blockPosition(), 4.0D)).stream().min(Comparator.comparingDouble(mob::distanceToSqr)).orElse(null);
    }
