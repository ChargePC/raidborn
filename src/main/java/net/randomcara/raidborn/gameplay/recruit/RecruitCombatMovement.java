...
public class RecruitCombatMovement {
    ...
    static boolean isDistanceFightingMob(Mob mob) {
        ...
        return id.equals(RaidbornCompatEntities.MC_ILLUSIONER) || id.equals(RaidbornCompatEntities.SANDR_ICEOLOGER) || id.equals(RaidbornCompatEntities.IINV_ALCHEMIST) || id.equals(RaidbornCompatEntities.IINV_ARCHIVIST) || id.equals(RaidbornCompatEntities.IINV_SORCERER) || id.equals(RaidbornCompatEntities.IINV_INVOKER)
                || id.equals(RaidbornCompatEntities.IINV_FIRECALLER) || id.equals(RaidbornCompatEntities.IINV_NECROMANCER) || id.equals(RaidbornCompatEntities.EWM_ENCHANTER);
    }
    ...
    static void applyDistanceCombatMovement(Mob mob, LivingEntity target, double approachSpeed, double preferredStopDistSqr) {
        ...
        if (distSqr < CASTER_MIN_COMBAT_DISTANCE_SQR) {
            if (mob instanceof PathfinderMob pathfinderMob) {
                Vec3 retreatPos = DefaultRandomPos.getPosAway(pathfinderMob, CASTER_RETREAT_HORIZONTAL_RANGE, CASTER_RETREAT_VERTICAL_RANGE, target.position());
                if (retreatPos != null) {
            ...
            if (away.lengthSqr() > 1.0E-4D) {
                away = away.normalize();
                mob.getNavigation().moveTo(mob.getX() + away.x * 4.0D, mob.getY(), mob.getZ() + away.z * 4.0D, approachSpeed);
                return;
            }
        }
